...
BEGIN
	DROP TABLE IF EXISTS TEM.MONDA_PELA;   	

	CREATE TABLE TEM.MONDA_PELA AS 
	SELECT 
		A.TIPO_DOCUMENTO, 
		...
		A.DOCUMENTO,
		C.NUM_OS,
		'N'::VARCHAR  AS PROCESADO
	FROM 
		FIN.CXP_DOC		AS A
		INNER JOIN FIN.CXP_ITEMS_DOC AA ON (AA.TIPO_DOCUMENTO=A.TIPO_DOCUMENTO AND A.DOCUMENTO=AA.DOCUMENTO)
		INNER JOIN OPAV.ACCIONES 	AS B	ON (AA.REFERENCIA_1 = B.ID_ACCION)
		JOIN OPAV.OFERTAS 	AS C	ON (B.ID_SOLICITUD = C.ID_SOLICITUD)
	...
	GROUP BY 
		...
		A.DOCUMENTO,
		C.NUM_OS
	ORDER BY C.NUM_OS,A.PROVEEDOR,A.DOCUMENTO;

	-- UPDATE TEM.MONDA_PELA SET WHERE DOCUMENTO=? AND TIPO_DOCUMENTO=? AND PROVEEDOR=? AND NUM_OS=? ;
...
--delete from con.mc_sl_fac_cc_sel where MC_____CODIGO____TD_____B = 'CXPN' and MC_____CODIGO____CD_____B = 'CFSL' and  procesado='S'
-- SELECT con.interfaz_sl_cdc_contratistas_selectrik_apoteosys()
--select * from con.mc_sl_fac_cc_sel where MC_____CODIGO____TD_____B = 'CXPN' and MC_____CODIGO____CD_____B = 'CFSL' and mc_____numero____b=70214 and  procesado='R'
-- update con.mc_sl_fac_cc_sel set MC_____REFERENCI_B='FOMS12328-17', procesado='N'
-- where MC_____CODIGO____TD_____B = 'CXPN' and MC_____CODIGO____CD_____B = 'CFSL' and procesado='R'
		...
		/**BUSCAMOS LA INFORMACION COMPLETA QUE CONFORMARA EL ASIENTO CONTABLE PARA MANDARLO A APOTEOSYS*/
				...
				FOR INFOITEMS_ IN 			        
						...
						SELECT * FROM (
								...
								AND C.NUM_OS=FACTURA_NM.NUM_OS
...
-- 					END IF;
				...
				IF(SW = 'S')THEN
					...
					SET 
						PROCESADO='S'
					WHERE 
						DOCUMENTO=FACTURA_NM.DOCUMENTO  AND 
						TIPO_DOCUMENTO='FAP' AND 
						NIT=FACTURA_NM.NIT AND 
						NUM_OS=FACTURA_NM.NUM_OS ;
...
-- 			END LOOP;
	...
	--AQUI MIRA PARA QUE VEAS BORRIS CARE MONDA...

	--SELECT DOCUMENTO FROM TEM.MONDA_PELA WHERE DOCUMENTO NOT IN (SELECT DOCUMENTO FROM TEM.MONDA_PELA  WHERE PROCESADO='N')  ORDER BY DOCUMENTO;

	

-- 	DELETE FROM 
-- 	con.mc_sl_fac_cc_sel 
-- 	where 
-- 	MC_____CODIGO____CONTAB_B='SELE' and 
-- 	MC_____CODIGO____TD_____B='CXPN' and 
-- 	MC_____CODIGO____CD_____B='CFSL' and 
-- 	MC_____NUMDOCSOP_B in(SELECT 
-- 				DOCUMENTO 
-- 			      FROM 
-- 				TEM.MONDA_PELA 
-- 			      WHERE 
-- 				DOCUMENTO IN (SELECT DOCUMENTO FROM TEM.MONDA_PELA  WHERE PROCESADO='N')  GROUP BY DOCUMENTO);

	UPDATE FIN.CXP_DOC SET PROCESADO = 'S'
	WHERE DOCUMENTO IN (SELECT DOCUMENTO FROM TEM.MONDA_PELA WHERE DOCUMENTO NOT IN (SELECT DOCUMENTO FROM TEM.MONDA_PELA  WHERE PROCESADO='N') )
	AND TIPO_DOCUMENTO = 'FAP' 
	AND HANDLE_CODE = 'CD'
	AND REG_STATUS =''
	AND DESCRIPCION LIKE 'Reunifica facturas internas:%'
