-- Function: select con.interfaz_sl_contratistas_nc_factoring_selectrik_apoteosys()
	...
	LOOP		
		...
		FOR INFOITEMS_ IN 
				...
				WHERE 
					...
					AND AA.HANDLE_CODE		=	'CD' 
					AND AA.DESCRIPCION		LIKE 'Aplica factoring y/o formula de la accion:%'
					AND AA.PERIODO			= '201710'--FACTURA_NM.PERIODO
					AND AA.DOCUMENTO 		= '385' --FACTURA_NM.DOCUMENTO ---VAS A PASAR EL DOCUMENTO 
					AND A.REFERENCIA_1		= '9039923' --FACTURA_NM.REFERENCIA_1 --VAS A PASAR EL I_ACCION
				GROUP BY 
					A.TIPO_DOCUMENTO,
					A.PROVEEDOR,
					CMC.CUENTA
				...
				SELECT 
					...
					A.PROVEEDOR,					
					CASE WHEN ROUND(A.VLR)<0 THEN ROUND(A.VLR*-1) ELSE 0 END AS VALOR_DEB, 
					CASE WHEN ROUND(A.VLR)>0 THEN ROUND(A.VLR) ELSE 0 END AS  VALOR_CREDT,					
