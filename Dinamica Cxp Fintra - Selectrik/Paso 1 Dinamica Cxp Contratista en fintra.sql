...
CUENTAS_DEBITO VARCHAR[]    := '{28151001,28151006}'; 
CUENTAS_CREDITO_1 VARCHAR[] := '{28151002,I010120064194,I010120064195,22050801}'; 
CUENTAS_CREDITO_2 VARCHAR[] := '{28151001,28151006,28151002,I010120064194,I010120064195}'; 
		...
		FOR INFOITEMS_ IN			
			...
			--SELECT SUM(VALOR_CREDT) as credito , SUM(VALOR_DEB) as debito FROM (
				SELECT  A.TIPO_DOCUMENTO,
					...
					WHERE       A.TIPO_DOCUMENTO	= 	'FAP'
						...
						AND A.DOCUMENTO 	=	'278'--FACTURA_NM.DOCUMENTO --'278'  
						AND A.PROVEEDOR 	= 	'9000742958'--FACTURA_NM.NIT  --'9000742958' 
						AND A.REG_STATUS	=	''
						AND A.CODIGO_CUENTA     = 	ANY (CUENTAS_DEBITO) --IN('28151001','28151006')						
			--)K
			--FIN DEBITO			
				UNION ALL
			...
			--SELECT SUM(VALOR_CREDT) AS CREDITO , SUM(VALOR_DEB) AS DEBITO FROM (
				...
				WHERE 	A.TIPO_DOCUMENTO	= 	'FAP'
					...
					AND A.PROVEEDOR 	= 	'9000742958'--FACTURA_NM.NIT --'9000742958'
					AND A.REG_STATUS	=	''
					AND A.CODIGO_CUENTA 	=       ANY(CUENTAS_CREDITO_1)	--('28151002','I010120064194','I010120064195','22050801')
			...
			--SELECT SUM(VALOR_CREDT) AS CREDITO , SUM(VALOR_DEB) AS DEBITO FROM (
				...
				SELECT  	A.TIPO_DOCUMENTO,
					...
					WHERE       A.TIPO_DOCUMENTO	= 	'FAP'
						...
						AND A.PROVEEDOR 	= 	'9000742958'--FACTURA_NM.NIT --'9000742958'
						AND A.REG_STATUS	=	''
						AND A.CODIGO_CUENTA     = 	ANY(CUENTAS_CREDITO_2)	--('28151001','28151006','28151002','I010120064194','I010120064195')
					GROUP BY 
						A.TIPO_DOCUMENTO,
						A.PROVEEDOR,				
						CCS.ID_SOLICITUD,
