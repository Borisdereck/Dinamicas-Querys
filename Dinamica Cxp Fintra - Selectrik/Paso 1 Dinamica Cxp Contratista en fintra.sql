...
BEGIN
	...
	FOR FACTURA_NM IN 
		...
		WHERE  
		  ...
		  --AND COALESCE(INDICADOR_TRASLADO_FINTRA, 'N' ) = 	'N'
		  AND A.DOCUMENTO NOT IN (SELECT DOCUMENTO FROM TEM.TRASLADO_CXP_CONTRATISTAS_FINTRA_SELECTRIK WHERE HANDLE_CODE = 'CD')
	...
	/*	
		...
		LEFT JOIN sl_centro_costos_selectrik_dblink AS CCS     ON (X.REFERENCIA_1 = CCS.ID_ACCION )
		WHERE  
		  A.TIPO_DOCUMENTO 			= 'FAP'
		  AND A.DOCUMENTO 			= '278' 
		  AND A.HANDLE_CODE 			= 'CD'
			...
			--SELECT SUM(VALOR_CREDT) as credito , SUM(VALOR_DEB) as debito FROM (
				SELECT  A.TIPO_DOCUMENTO,
					...
					WHERE       A.TIPO_DOCUMENTO		= 	'FAP'
						...
						AND A.DOCUMENTO 		=	'278'
						AND A.CODIGO_CUENTA IN('28151001','28151006')						
				--)K
...
-- 			END IF;
		...
		IF(SW = 'S')THEN
			--UPDATE FIN.CXP_DOC SET PROCESADO = 'S' WHERE DOCUMENTO =  FACTURA_NM.DOCUMENTO AND TIPO_DOCUMENTO = 'FAP' AND HANDLE_CODE = 'CD' ;
			
			INSERT INTO TEM.TRASLADO_CXP_CONTRATISTAS_FINTRA_SELECTRIK(PERIODO , DOCUMENTO, HANDLE_CODE, CONCEPTO, PROCESADO, CREATION_DATE , CREATION_USER, LAST_UPDATE , USER_UPDATE) 
			VALUES (FACTURA_NM.PERIODO, FACTURA_NM.DOCUMENTO , FACTURA_NM.HANDLE_CODE , MCTYPE.MC_____CODIGO____CD_____B , 'S', now(), 'BTERRAZA', now(), 'BTERRAZA');
